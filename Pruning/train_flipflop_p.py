...
#Only the layer names present in this list are trained/retrained
layers_to_train = ["sublayers.4.cudnn_gru.weight_ih_l0"]
...
def load_network(args, alphabet_info, res_info, log):
    ...
    for i in [1, 3, 5]:
        network.sublayers[i].layer.ApplyMask()
    for i in [2, 4]:
        network.sublayers[i].ApplyMask()
...
def train_model(
    ...
    for curr_iter in range(train_params.niteration):
        ...
        if len(loss_list) == 3:
            if (loss_list[0] + loss_list[1] + loss_list[2])/3.0 < 0.08:
                break
            loss_list.pop(0)
            loss_list.insert(2, fval)
        else:
            loss_list.append(fval)
        ...
        if (curr_iter + 1) % train_params.save_every == 0:
            ...
            if res_info.is_lead_process:
                for i in [1, 3, 5]:
                    net_info.net.sublayers[i].layer.ApplyMask()
                for i in [2, 4]:
                    net_info.net.sublayers[i].ApplyMask()
