...
class GruMod(nn.Module):
    ...
    def StoreMask(self, layer_name, mask):
        self.masks[layer_name] = mask
        self.mask_flag = True

    def ApplyMask(self):
        if(self.mask_flag == True):
            sd = self.cudnn_gru.state_dict()
            
            if next(self.cudnn_gru.parameters()).is_cuda:
                for layer in self.masks:
                    self.masks[layer] = self.masks[layer].cuda()
            
            for layer in self.masks:
                sd[layer].data *= self.masks[layer]

            self.cudnn_gru.load_state_dict(sd)
...
class GlobalNormFlipFlop(nn.Module):
    ...
    def ApplyMask(self):
        if(self.mask_flag == True):
            sd = self.linear.state_dict()
            
            if next(self.linear.parameters()).is_cuda:
                for layer in self.masks:
                    self.masks[layer] = self.masks[layer].cuda()
            ...
            self.linear.load_state_dict(sd)
