...
class GruMod(nn.Module):
    ...
    def __init__(self, insize, size, has_bias=True):
        ...
        self._disable_state_bias()
        self.reset_parameters()
        self.q_flag = [False, False, False, False]
        self.run_flag = True
        self.q_data = dict()
    ...
    def forward(self, x):
        ...
        Returns:
            :torch:`Tensor`: Output of layer
        """
        if self.run_flag and self.convert_flag:
            if self.q_flag[0]:
                self.cudnn_gru.weight_ih_l0.data = torch.nn.Parameter(self.q_data["weight_ih_l0"][0] * (self.cudnn_gru.weight_ih_l0.data.type(torch.float32) - self.q_data["weight_ih_l0"][1]))
            if self.q_flag[1]:
                self.cudnn_gru.weight_hh_l0.data = torch.nn.Parameter(self.q_data["weight_hh_l0"][0] * (self.cudnn_gru.weight_hh_l0.data.type(torch.float32) - self.q_data["weight_hh_l0"][1]))
            if self.q_flag[2]:
                self.cudnn_gru.bias_ih_l0.data = torch.nn.Parameter(self.q_data["bias_ih_l0"][0] * (self.cudnn_gru.bias_ih_l0.data.type(torch.float32) - self.q_data["bias_ih_l0"][1]))
            if self.q_flag[3]:
                self.cudnn_gru.bias_hh_l0.data = torch.nn.Parameter(self.q_data["bias_hh_l0"][0] * (self.cudnn_gru.bias_hh_l0.data.type(torch.float32) - self.q_data["bias_hh_l0"][1]))
            self.run_flag = False

        self.cudnn_gru.flatten_parameters()
        y, hy = self.cudnn_gru.forward(x)
        return y
...
class Convolution(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.run_flag and self.convert_flag:
            if self.q_flag[0]:
                self.conv.weight.data = torch.nn.Parameter(self.q_data["weight"][0] * (self.conv.weight.data.type(torch.float32) - self.q_data["weight"][1]))
            if self.q_flag[1]:
                self.conv.bias.data = torch.nn.Parameter(self.q_data["bias"][0] * (self.conv.bias.data.type(torch.float32) - self.q_data["bias"][1]))
            self.run_flag = False
        x = x.permute(1, 2, 0)
...
class GlobalNormFlipFlop(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.run_flag and self.convert_flag:
            if self.q_flag[0]:
                self.linear.weight.data = torch.nn.Parameter(self.q_data["weight"][0] * (self.linear.weight.data.type(torch.float32) - self.q_data["weight"][1]))
            if self.q_flag[1]:
                self.linear.bias.data = torch.nn.Parameter(self.q_data["bias"][0] * (self.linear.bias.data.type(torch.float32) - self.q_data["bias"][1]))
            self.run_flag = False
